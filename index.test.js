let AlphanumericEncoder = require('./index')
...
const expectedValidDictionaryValues = [
    'ABCD',
    'ABcd',
    'aBcD',
    'ABC123'
...
const expectedInvalidDictionaryValuesWithRepeatingSymbols = [
    'ABCDA',
    'ABCDa',
    'ABCD1231'
]

const expectedInvalidDictionaryValuesWithImproperSymbols = [
    'ABC!',
    'ABC@',
    'ABC?',
    '&#$%(*)'
...
describe('Dictionary Validation', () => {
    ...
    describe('Valid Dictionaries', () => {
        ...
        test.each(expectedValidDictionaryValues)('Expect %p to be a valid dictionary', (validDictionaryString) => {
    ...
    describe('Invalid Dictionaries', () => {
        ...
        test.each(expectedInvalidDictionaryValuesWithRepeatingSymbols)('Expect %p to be a be invalid dictionary and throw error due to repeating symbol', (invalidDictionaryString) => {
        ...
        test.each(expectedInvalidDictionaryValuesWithImproperSymbols)('Expect %p to be a be invalid dictionary due to non-alphanumeric symbol', (invalidDictionaryString) => {
...
describe('Test Encoding', () => {
    ...
    describe('Encode with Default Dictionary', () => {
        test.each(numberToEncodedLetters)('Under default dictionary, expect %p to encode to %p', (number, letter) => {
            expect(encoder.encode(number)).toBe(letter)
        })
    })

    describe('Encode with Custom Dictionary', () => {
        test.each(numberWithDictionaryToEncodedLetters)('Under dictionary %p, expect %p to encode to %p', (dictionary, number, letter) => {
...
describe('Test Decoding', () => {
    ...
    describe('Invalid Decode Values Using Default Dictionary', () => {
        ...
        const encodedWithNonexistentCharacter = [
            0,
            2,
            -15,
            -1.7,
        ...
        test.each(encodedWithNonexistentCharacter)('Under default dictionary, trying to decode %p will return "undefined" due to character not existing in dictionary', (letter) => {
            expect(encoder.decode(letter)).toBeUndefined()
        })
    })

    describe('Decode with Default Dictionary', () => {
        test.each(numberToEncodedLetters)('Under default dictionary, expect %p to be decoded from %p', (number, letter) => {
            expect(encoder.decode(letter)).toBe(number)
        })
    })  

    describe('Decode with Custom Dictionary', () => {
        test.each(numberWithDictionaryToEncodedLetters)('Under dictionary %p, expect %p to be decoded from %p', (dictionary, number, letter) => {
            encoder.dictionary = dictionary
            expect(encoder.decode(letter)).toBe(number)
        })
    })  
